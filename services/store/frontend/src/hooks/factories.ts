...
export function createQueryHook<TParams, TData>(
  ...
  defaultOptions?: Partial<UseQueryOptions<TData>>
) {
...
export function createInfiniteQueryHook<TParams, TData>(
  ...
  defaultOptions?: Partial<UseInfiniteQueryOptions<TData>>
) {
  return function useInfiniteDataQuery(
    params?: TParams,
    options?: Partial<UseInfiniteQueryOptions<TData>>
  ) {
...
// Standard pagination helper for infinite queries - supports normalized responses
export function createPaginationHelper<T>() {
  return {
    getNextPageParam: (lastPage: { data: T[]; pagination: any }) => {
